...
class HeightCachingVC: BaseVC, UITableViewDelegate, UITableViewDataSource {
    
    private var _heightCache: [Int: CGFloat] = [:]
    
    override init() {
        super.init()
        self.title = "HeightCachingVC"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let mediaItems = message.mediaItems {
            if (mediaItems.count == 1) { cellIdentifier = "StaticCells_MediaCell_1_\(message.direction.capitalized)" }
            if (mediaItems.count == 2) { cellIdentifier = "StaticCells_MediaCell_2_\(message.direction.capitalized)" }
            if (mediaItems.count == 3) { cellIdentifier = "StaticCells_MediaCell_3_\(message.direction.capitalized)" }
        }
